...
// ConsensusVersion defines the current custom_token module consensus version.
const ConsensusVersion = 1

// AppModuleBasic defines the basic application module used by the custom_token module.
type AppModuleBasic struct {
	cdc codec.Codec
}

// Name returns the custom_token module's name.
func (AppModuleBasic) Name() string {
	return customtokentypes.ModuleName
}

// RegisterLegacyAminoCodec registers the custom_token module's types on the given LegacyAmino codec.
func (AppModuleBasic) RegisterLegacyAminoCodec(cdc *codec.LegacyAmino) {
	customtokentypes.RegisterLegacyAminoCodec(cdc)
}

// RegisterInterfaces registers the module's interface types
func (a AppModuleBasic) RegisterInterfaces(reg codectypes.InterfaceRegistry) {
	customtokentypes.RegisterInterfaces(reg)
}

// DefaultGenesis returns default genesis state as raw bytes for the custom_token module.
func (AppModuleBasic) DefaultGenesis(cdc codec.JSONCodec) json.RawMessage {
	return cdc.MustMarshalJSON(customtokentypes.DefaultGenesisState())
}

// ValidateGenesis performs genesis state validation for the custom_token module.
func (AppModuleBasic) ValidateGenesis(cdc codec.JSONCodec, config client.TxEncodingConfig, bz json.RawMessage) error {
	var genState customtokentypes.GenesisState
	if err := cdc.UnmarshalJSON(bz, &genState); err != nil {
		return fmt.Errorf("failed to unmarshal %s genesis state: %w", customtokentypes.ModuleName, err)
	}
	return genState.ValidateGenesis()
}

// RegisterGRPCGatewayRoutes registers the gRPC Gateway routes for the custom_token module.
...
// AppModule implements an application module for the custom_token module.
...
func (am AppModule) RegisterServices(cfg module.Configurator) {
	customtokentypes.RegisterMsgServer(cfg.MsgServer(), keeper.NewMsgServerImpl(am.keeper))
	customtokentypes.RegisterQueryServer(cfg.QueryServer(), keeper.NewQueryServerImpl(am.keeper))
}

// InitGenesis performs genesis initialization for the custom_token module.
func (am AppModule) InitGenesis(ctx sdk.Context, cdc codec.JSONCodec, gs json.RawMessage) []abci.ValidatorUpdate {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			panic(fmt.Sprintf("custom_token InitGenesis panic: %s", panicMsg))
		}
	}()
	...
	if err := cdc.UnmarshalJSON(gs, &genState); err != nil {
		...
		ctx.Logger().Error("Failed to unmarshal genesis state during InitGenesis",
			"module", customtokentypes.ModuleName,
			"error", err.Error())
		panic(fmt.Sprintf("failed to unmarshal %s genesis state: %s", customtokentypes.ModuleName, err.Error()))
	}
	...
	if err := genState.ValidateGenesis(); err != nil {
		...
		genState.Params = customtokentypes.DefaultParams()
	}
...
// ExportGenesis returns the exported genesis state as raw bytes for the custom_token module.
...
// BeginBlock returns the begin blocker for the custom_token module.
func (am AppModule) BeginBlock(ctx sdk.Context) {
	am.keeper.BeginBlocker(ctx)
}

// EndBlock returns the end blocker for the custom_token module.
